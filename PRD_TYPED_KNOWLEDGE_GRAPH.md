# VexClaw — Typed Knowledge Graph Upgrade (Post-MVP Roadmap)

## 1) Objective

Move VexClaw memory from **memory-store-centric documents** to a **typed knowledge graph** while preserving the current MCP tool contract and existing memory data.
...
- ✅ `vexclawAssociations` (VexClaw association store) already stores undifferentiated graph links between memories.
- ✅ Recall can include spread activation from linked memories (`includeAssociations` path already implemented in `convex/vexclaw/recall.ts`).
- ✅ **Phase 0 foundation is implemented**:
  - `vexclawNodes`, `vexclawRelations`, `vexclawMemoryNodeLinks` are added to `convex/schema.ts`.
  - `convex/vexclaw/graph.ts` includes `seedKnowledgeGraphFromMemory` and `getKnowledgeGraphFoundationStatus`.
...
- `createdAt: number`
- `updatedAt: number`
- `sourceMemoryIds: array<vexclawMemories>` (physical table currently named `vexclawMemories`)
- `status: "active" | "deprecated"`

### 3.2 Edges table: `vexclawRelations`

Purpose: typed facts between nodes and memory evidence.

Fields:
- `fromNodeId: id<vexclawNodes>`
- `toNodeId: id<vexclawNodes>`
- `relationType:`
  - "mentions"
  - "decided_in"
  - "leads_to"
...
- `weight: number (0–1)`
- `evidenceMemoryIds: array<vexclawMemories>` (physical table currently named `vexclawMemories`)
...
- `createdAt: number`
- `updatedAt: number`
- `promotedFrom: optional id<vexclawRelations>` (if this edge replaced an older one)

### 3.3 Bridge table: `vexclawMemoryNodeLinks` (derived/managed)

Purpose: stable bidirectional mapping for explainability and fast UIs.

Fields:
- `memoryId: id<vexclawMemories>` (physical table currently named `vexclawMemories`)
- `nodeId: id<vexclawNodes>`
...
### 4.1 Capture path
Every `vexclaw_remember` call should optionally include `graphIntents`:
- inline tags (`tags`),
...
Create `convex/vexclaw/graph.extractFromText`:
...
- Association cron (`vexclaw-associate`) can continue to seed with similarity-based `co_occurred` until graph builder reaches steady state.
...
- ✅ Add new tables (`vexclawNodes`, `vexclawRelations`, `vexclawMemoryNodeLinks`)
- ✅ Create migration job to seed graph:
  ...
  - edges from existing `vexclawAssociations` converted into typed `supports/conflicts_with/depends_on/occurs_with/leads_to` relations
...
- Extend `vexclaw/memories.createMemory` and `vexclaw_remember` to emit graph mutations in the same flow.
- Add idempotency keys per text chunk to avoid duplicate nodes/edges.

### Phase 2 (1–2 days): query and MCP enhancements
- Add `vexclaw_neighbors` and `vexclaw_graph_explain` for dashboards/agents.
...
- Keep old `vexclawAssociations` behavior for one release.
- Dual-write edges to both `vexclawAssociations` and `vexclawRelations` while we stabilize.
...
- [ ] `vexclaw_wake` includes at least one relation-aware suggestion (goal + blockers + dependencies).
- [ ] Recall result can include `vexclaw_relation_trace` evidence IDs for every relation-backed memory.
- [ ] Rollback path: disabling graph features returns to legacy `vexclawAssociations` only with no data loss.
