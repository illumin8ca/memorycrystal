    ...
    <div>
      ...
      <div className="space-y-2">
        {!messages ? (
          ...
          <div className="text-secondary text-sm text-center py-12 font-mono">No messages yet.</div>
        ) : (
          messages.map((m) => (
            <div key={m._id} className="bg-surface border border-white/[0.07] p-3 sm:p-4 flex flex-col sm:flex-row gap-2 sm:gap-4 sm:items-start min-w-0">
              <div className="flex items-center gap-2 sm:contents">
                <span className="neon-text text-xs font-mono shrink-0 sm:w-20 sm:pt-0.5">{formatTime(m.timestamp)}</span>
                <span
