...
export const getMemoriesForCleanup = query({
  ...
  handler: async (ctx, args) => {
    return ctx.db.query("vexclawMemories").take(args.limit);
  },
});

export const getAssociationsByFrom = query({
  args: { memoryId: v.id("vexclawMemories"), limit: v.number() },
  handler: async (ctx, args) => {
    return ctx.db
...
export const deleteAssociation = mutation({
  args: { associationId: v.id("vexclawAssociations") },
  handler: async (ctx, args) => {
...
const deleteAssociationsForMemory = async (ctx: any, memoryId: string) => {
  const outgoing = await ctx.runQuery("vexclaw/cleanup:getAssociationsByFrom" as any, {
    memoryId,
    limit: 200,
  });

  const incoming = await ctx.runQuery("vexclaw/cleanup:getAssociationsByTo" as any, {
    memoryId,
    limit: 200,
  });

  for (const association of [...outgoing, ...incoming]) {
    await ctx.runMutation("vexclaw/cleanup:deleteAssociation" as any, {
...
export const runCleanup = action({
  ...
  handler: async (ctx, args) => {
    ...
    const memoryBatch = await ctx.runQuery("vexclaw/cleanup:getMemoriesForCleanup" as any, {
      limit: MAX_BATCH + 1,
    });
    ...
    for (const memory of memories) {
      try {
        ...
        if (isExpiredSensory) {
          ...
          await ctx.runMutation("vexclaw/cleanup:deleteMemory" as any, {
            memoryId: memory._id,
          });
          deletedSensory += 1;
          continue;
        ...
        if (isWeakMemory) {
          await ctx.runMutation("vexclaw/cleanup:archiveWeakMemory" as any, {
            memoryId: memory._id,
            archivedAt: now,
          });
