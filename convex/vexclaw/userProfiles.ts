...
export const createOrGet = mutation({
  ...
  handler: async (ctx, { userId }) => {
    const existing = await ctx.db
      .query("vexclawUserProfiles")
      .withIndex("by_user", (q) => q.eq("userId", userId))
      .first();
    if (existing) return existing;
    const now = Date.now();
    const id = await ctx.db.insert("vexclawUserProfiles", {
      userId,
...
export const getByUser = query({
  args: { userId: v.string() },
  handler: async (ctx, { userId }) =>
    ctx.db.query("vexclawUserProfiles").withIndex("by_user", (q) => q.eq("userId", userId)).first(),
});

export const getByPolarCustomer = query({
  args: { polarCustomerId: v.string() },
  handler: async (ctx, { polarCustomerId }) =>
    ctx.db.query("vexclawUserProfiles").withIndex("by_polar_customer", (q) => q.eq("polarCustomerId", polarCustomerId)).first(),
});

export const getByPolarSubscription = query({
  args: { polarSubscriptionId: v.string() },
  handler: async (ctx, { polarSubscriptionId }) =>
    ctx.db.query("vexclawUserProfiles").withIndex("by_polar_subscription", (q) => q.eq("polarSubscriptionId", polarSubscriptionId)).first(),
});

export const updateSubscription = mutation({
  args: {
    userProfileId: v.id("vexclawUserProfiles"),
