...
function getEmbeddingProvider(): EmbeddingProvider {
  return (process.env.EMBEDDING_PROVIDER ?? "gemini").toLowerCase() === "openai" ? "openai" : "gemini";
}
...
async function requestOpenAIEmbedding(apiKey: string, text: string): Promise<number[] | null> {
...
async function embedAssetDoc(asset: {
  ...
  if (provider === "gemini") {
    ...
    if (asset.kind === "text") {
      return requestGeminiEmbeddingForText(apiKey, fallbackText);
    }

    return requestGeminiEmbeddingForAsset(apiKey, asset);
  }
  ...
  return requestOpenAIEmbedding(apiKey, fallbackText);
}
