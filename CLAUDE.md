...
2. **SaaS dashboard** — Next.js 16 + Convex web app with Polar.sh billing, deployed to Railway
...
cd mcp-server && npm run start        # start MCP server (stdio)
...
|---|---|---|
| `apps/web/` | SaaS dashboard | Next.js 16, React 19, Tailwind 4, Convex Auth, Polar.sh SDK |
| `convex/` | Backend (shared by all consumers) | Convex (schema, mutations, queries, crons, auth) |
| `convex/crystal/` | All Memory Crystal Convex functions | 16 modules: memories, recall, capture, sessions, wake, graph, etc. |
| `mcp-server/` | MCP server (8 tools) | TypeScript, @modelcontextprotocol/sdk, OpenAI SDK |
...
AI response → capture-hook.js (llm_output)
  ...
  → save raw messages to STM (crystalMessages, 14-day TTL)
...
- **STM** (`crystalMessages`) — raw verbatim messages, 14-day TTL, vector-indexed
...
- Background `#090909`, Surface `#141414`, Elevated `#1e1e1e`
- Accent `#0066ff`, Accent hover `#0044cc`
...
- Memory Crystal is proprietary — do not publish to ClaWHub or any public registry
